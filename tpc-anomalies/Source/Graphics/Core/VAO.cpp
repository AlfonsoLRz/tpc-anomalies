...
VAO::VAO(bool gpuGeometry): _vao(-1), _vbo(RendEnum::numVBOTypes()), _ibo(RendEnum::numIBOTypes())
{
	...
	if (!gpuGeometry)
	{
		this->defineSimpleVBO(RendEnum::VBO_POSITION, sizeof(vec4), GL_FLOAT, RendEnum::VBO_POSITION);
		this->defineSimpleVBO(RendEnum::VBO_NORMAL, sizeof(vec3), GL_FLOAT, RendEnum::VBO_NORMAL);
		this->defineSimpleVBO(RendEnum::VBO_TEXT_COORD, sizeof(vec2), GL_FLOAT, RendEnum::VBO_TEXT_COORD);
		this->defineSimpleVBO(RendEnum::VBO_TANGENT, sizeof(vec3), GL_FLOAT, RendEnum::VBO_TANGENT);
	}
	else
	{
...
void VAO::defineSimpleVBO(const RendEnum::VBOTypes vboType, const GLsizei dataSize, const GLuint unitType, const GLuint shaderSlot)
{
...
void VAO::defineInterleavedVBO(const RendEnum::VBOTypes vboType, const GLsizei totalSize, const std::vector<unsigned long long>& dataSize, const std::vector<GLuint>& unitType, const GLuint shaderSlot)
{
	// VAO is considered to be binded at this point
	...
	{
		glEnableVertexAttribArray(shaderSlot);
		glVertexAttribPointer(shaderSlot + i, (GLint) (dataSize[i] / sizeof(unitType[i])), unitType[i], GL_FALSE, totalSize, ((GLubyte*)nullptr + (accumSize)));
