...
protected:
	...
	*/
	void defineSimpleVBO(const RendEnum::VBOTypes vboType, const GLsizei dataSize, const GLuint unitType, const GLuint shaderSlot);
	...
	*	@param shaderSlot Input channel of VBO.
	*/
	void defineInterleavedVBO(const RendEnum::VBOTypes vboType, const GLsizei totalSize, const std::vector<unsigned long long>& dataSize, const std::vector<GLuint>& unitType, const GLuint shaderSlot);
...
public:
	...
	*/
	template<typename T>
	int defineVBO(const RendEnum::VBOTypes vboType, const T dataExample, const GLuint openGLBasicType);
	...
	*/
	...
	int defineMultiInstancingVBO(const RendEnum::VBOTypes vboType, const T dataExample, const Z dataPrimitive, const GLuint openGLBasicType);
...
template<typename T>
inline int VAO::defineVBO(const RendEnum::VBOTypes vboType, const T dataExample, const GLuint openGLBasicType)
{
	glBindVertexArray(_vao);

	this->defineSimpleVBO(vboType, sizeof(dataExample), openGLBasicType, vboType);

	return vboType;
}

template<typename T, typename Z>
inline int VAO::defineMultiInstancingVBO(const RendEnum::VBOTypes vboType, const T dataExample, const Z dataPrimitive, const GLuint openGLBasicType)
{
	glBindVertexArray(_vao);

	// VBOs
	glGenBuffers(1, &_vbo[vboType]);
	glBindBuffer(GL_ARRAY_BUFFER, _vbo[vboType]);

	// Offset
	glEnableVertexAttribArray(vboType);
	glVertexAttribPointer(vboType, sizeof(dataExample) / sizeof(dataPrimitive), openGLBasicType, GL_FALSE, sizeof(dataExample), (GLubyte*)nullptr);
	glBindBuffer(GL_ARRAY_BUFFER, 0);
	glVertexAttribDivisor(vboType, 1);
